...
class AffineTransform(nn.Module):
    ...
    """
    def __init__(self, degrees=5, translate=0.01, scale=1.0, shear=None):
        super(AffineTransform, self).__init__()
        self.trs = kornia.augmentation.RandomAffine(degrees, (translate, translate), (scale, scale), shear, p=1, return_transform=True)
...
class ElasticTransform(nn.Module):
    ...
    def __init__(self, kernel_size: int = 63, sigma: float = 16, alpha: Tuple[float, float]= (1.0, 1.0), align_corners: bool = False, mode: str = "bilinear"):
...
def get_random_affine_coeff():
    ...
    degree = random.uniform(0, 0) # light_defo: (0, 0); middle_defo: (0, 5); heavy_defo: (0, 10)
    translate = random.uniform(0.0, 0.01) # light_defo: (0.0, 0.01); middle_defo: (0.0, 0.02); heavy_defo: (0.02, 0.05)
    scale = random.uniform(1.0, 1.0)
    shear = None
    return degree, translate, scale, shear

def get_random_deformable_coeff():
    kernel = random.randrange(97, 101, 2)
    sigma = random.randint(32, 32) # light_defo: (32, 32); middle_defo: (16, 36); heavy_defo: (16, 24)
...
if __name__ == '__main__':
    ...
    ir_list = [x for x in sorted(ir_folder.glob('*')) if x.suffix in ['.png', '.jpg', '.bmp', '.tiff']]
    vi_list = [x for x in sorted(vi_folder.glob('*')) if x.suffix in ['.png', '.jpg', '.bmp', '.tiff']]

    # TODO: save path
    ir_affine_path          = './dataset/train/ir_affine'
    ir_affine_deform_path   = './dataset/train/ir_warp'
    disp_affine_deform_path = './dataset/train/disp_affine_deform'
